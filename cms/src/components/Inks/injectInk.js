...
import { getOffset, isTouchDevice } from 'core/util/index';
import { LEFT_MOUSE } from 'core/util/keyCodes';
...
export default ComposedComponent => class Ink extends PureComponent {
  ...
  render() {
    ...
    const inks = this.state.inks.map(({ time, ...inkProps }) =>
    <InkTransition key={ time.getTime() } { ...inkProps } />);
    const ink = (
